...
const httpFormatter = format.combine(
    format.printf((info) => {
        return `[${info.label || ""}]\t${info.timestamp}\t[${info.level || ""}]\t[${info.type || ""}]\t[${info.ip || ""}]\t[${
            info.method || ""
        }]\t${info.path || ""}\t[${info.identify || ""}] \t---- ${JSON.stringify(info.message)} ;`;
    }),
);
...
const sqlFormatter = format.combine(
    format.printf(
        (info) =>
            `[${info.label || ""}]\t${info.timestamp}\t[${info.level || ""}]\t[${info.type || "SQL"}]\t${
...
loggers.add("app", {
    format: format.combine(format.label({ label: "Sambytech" }), format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), appFormatter),
    transports: [
        new transports.File({
            level: "error",
...
loggers.add("http", {
    format: format.combine(format.label({ label: "HTTP" }), format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), httpFormatter),
    transports: [
        new transports.File({
            level: "info",
...
loggers.add("db", {
    format: format.combine(format.label({ label: "DATABASE" }), format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), sqlFormatter),
    transports: [
        new transports.File({
            level: "error",
